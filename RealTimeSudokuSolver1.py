...
#another method to find corner . source-google
def find_corners_of_largest_polygon(img):
	"""Finds the 4 extreme corners of the largest contour in the image."""
	contours, h = cv2.findContours(img.copy(), cv2.RETR_TREE, cv2.CHAIN_APPROX_SIMPLE)  # Find contours
...
def recognize_and_solve_sudoku(image, model, old_sudoku):
    ...
    blur = cv2.GaussianBlur(gray, (5,5), 0)   # // imgae, kernal, sigma xvalue
    ...
    # all_contours = cv2.drawContours(thresh.copy(), contours, -1, (255, 0, 0), 2)
    # cv2.imshow('s',all_contours)
    #// contours is a python list of all the contours in the image. 
    #//...each indivdual contour is a numpy array of (x,y) coordinates of the boundry  
    ...
    # corners = find_corners_of_largest_polygon(thresh)
    ...
    # cv2.imshow('adaptive warp img',warp)
    warp = cv2.bitwise_not(warp)
    
    # cv2.imshow('bitwise warp img',warp)
